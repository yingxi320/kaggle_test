...
class DataReader(object):
    ...
    def batch_generator(self, batch_size, df, mode, shuffle=True, num_epochs=1000):
        ...
        for batch in batch_gen:
            ...
            for i, (data, data_raw, start_idx, x_lag, xy_lag, holi, uid) in enumerate(zip(
                    batch['x'], batch['x_raw'], batch['start_date'], batch['x_lags'],
                    batch['xy_lags'], batch['holidayinfo'], batch['id']
                )
            ):
                ...
                y_holi[i, :rand_decode_len] = holi[end_idx: end_idx + rand_decode_len]
...
class cnn(TFBaseModel):
    ...
    def get_input_sequences(self):
        ...
        self.y_lags = tf.placeholder(tf.float32, [None, self.num_decode_steps, 7])
        ...
        self.encode_features = tf.concat([
            ...
            tf.tile(tf.expand_dims(tf.one_hot(self.air_area_name, self.reader.num_area_name), 1), (1, tf.shape(self.x)[1], 1)),
            tf.tile(tf.expand_dims(tf.one_hot(self.air_genre_name, self.reader.num_genre_name), 1), (1, tf.shape(self.x)[1], 1)),
            tf.tile(tf.expand_dims(tf.one_hot(self.air_store_id2, self.reader.num_store_id), 1), (1, tf.shape(self.x)[1], 1)),
        ], axis=2)
        ...
        decode_features = tf.concat([
            ...
            tf.tile(tf.expand_dims(tf.one_hot(self.air_area_name, self.reader.num_area_name), 1), (1, self.num_decode_steps, 1)),
            tf.tile(tf.expand_dims(tf.one_hot(self.air_genre_name, self.reader.num_genre_name), 1), (1, self.num_decode_steps, 1)),
            tf.tile(tf.expand_dims(tf.one_hot(self.air_store_id2, self.reader.num_store_id), 1), (1, self.num_decode_steps, 1)),
    ...
    def encode(self, x, features):
        ...
        conv_inputs = [h]
        for i, (dilation, filter_width) in enumerate(list(zip(self.dilations, self.filter_widths))[:-1]):
