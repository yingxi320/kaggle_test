...
visit_data['visitors'] = visit_data['visitors'].apply(np.log1p)
...
test_visit_var = sample_submission['visitors'].apply(np.expm1)
...
X_tr, X_te, y_tr, y_te = train_test_split(X, y, test_size=0.2, random_state=2018, shuffle=False)
...
print("\n".join(("%s: %.2f" % x) for x in sorted(
    zip(train.columns, bst.feature_importance("gain")),
    key=lambda x: x[1], reverse=True
)))
print('RMSE train: ', RMSLE(y_tr, bst.predict(X_tr, num_iteration=bst.best_iteration or MAX_ROUNDS)))
print('RMSE validate: ', RMSLE(y_te, bst.predict(X_te, num_iteration=bst.best_iteration or MAX_ROUNDS)))
