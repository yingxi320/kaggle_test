...
class Model(object):
    ...
    def evaluate(self, X_val, y_val):
        ...
        result = mean_squared_error(y_val, guessed_sales) ** 0.5
        return result
...
class LinearModel(Model):
    ...
    def guess(self, feature):
        return np.expm1(self.clf.predict(feature))
...
class SVM(Model):
    ...
    def __init__(self, X_train, y_train, X_val, y_val):
        ...
        self.clf.fit(self.X_train, np.log1p(self.y_train))
        print("Result on validation data: ", self.evaluate(X_val, y_val))
...
class XGBoost(Model):
    ...
    def guess(self, feature):
        ...
        return np.expm1(self.bst.predict(dtest))

class Lightgbm(Model):
    def __init__(self, X_train, y_train, X_val, y_val):
        super().__init__()
        dtrain = lgb.Dataset(X_train, label=np.log1p(y_train))
        dval = lgb.Dataset(X_val, label=np.log1p(y_val))
        ...
        num_round = 8000
        self.bst = lgb.train(param, dtrain, num_boost_round=num_round, valid_sets=evallist,
...
class KNN(Model):
    ...
    def guess(self, feature):
        return np.expm1(self.clf.predict(self.normalizer.transform(feature)))
...
class NN_with_EntityEmbedding(Model):
    ...
    def _val_for_fit(self, val):
        val = np.log1p(val) / self.max_log_y
        return val

    def _val_for_pred(self, val):
        return np.expm1(val * self.max_log_y)
